...
export const knapsack = (
  ...
  // To find the selected items, we traceback from dp[numberOfItems][capacity]
  const selectedItems: number[] = [];
  let w = capacity;
  for (let i = numberOfItems; i > 0 && w > 0; i--) {
    if (dp[i][w] !== dp[i - 1][w]) {
  ...
  return { maxValue: dp[numberOfItems][capacity], selectedItems: selectedItems.reverse() }
}
